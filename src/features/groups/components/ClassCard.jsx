    ...
    <>
      ...
      >
        ...
        {pastMembers.length > 0 && (
          <div className={`pt-3 ${sharedMembers.length > 0 ? "mt-3" : ""} border-t border-slate-200`}>
            <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              {pastMembers.slice(0, 3).map((member) => (
                <span
                  key={member.id}
                  className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 text-xs font-medium"
                  title={member.name}
                >
                  {member.name.charAt(0).toUpperCase()}
                </span>
              ...
              {pastMembers.length > 3 && (
                <span className="w-6 h-6 rounded-full bg-slate-100/70 flex items-center justify-center text-slate-600 text-xs font-medium">
                  +{pastMembers.length - 3}
                </span>
              )}
            </div>
          ...
          <div className="bg-white border border-gray-300 rounded-lg max-w-lg bg-white w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-600">{classInfo.code}</span>
                <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-600 font-medium">
                  {classInfo.credits} credits
                </span>
              </div>
                    ...
                    <ul className="space-y-2" role="list">
                      {sharedMembers.map((member) => (
                        <li key={member.id} className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-600 capitalize">({member.role})</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                  ...
                  {pastMembers.length > 0 ? (
                    <ul className="space-y-2" role="list">
                      {pastMembers.map((member) => (
                        <li key={member.id} className="flex items-center gap-2">
                          <span className="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 text-xs font-medium">
                            {member.name.charAt(0).toUpperCase()}
                          </span>
                          <span className="text-sm">{member.name}</span>
                          {member.semester && (
                            <span className="text-xs text-slate-600">({member.semester})</span>
                          )}
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-slate-600">No group members have taken this class before</p>
                  )}
                </div>
              </div>
            </div>
